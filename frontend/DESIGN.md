---
version: beta
name: OCP-auth-brand-system
description: OCP Auth uses a focused online-course brand system: a deep green editorial hero, an OCP book-logo lockup, warm white form surfaces, gold and sky accents, compact rounded-rectangle controls, safe cookie-auth UX, and role-aware placeholder dashboards for session validation.
...
## Current Auth UI Contract

This section is binding for `feat-auth`. The longer notes below remain design inspiration, but Auth screens should follow this contract when the two conflict.

- **Brand mark**: Use the OCP lockup made from a rounded learning-screen icon with a play marker and graduation cap plus `OCP` and `Online Course Platform`.
- **Auth hero**: Use a deep green hero surface with subtle gold and sky accents. The left side must include the logo, a short learning-focused headline support line, and concise platform highlights.
- **Form surface**: Keep the right panel white, quiet, and form-first. Inputs use 6px radius, primary/secondary buttons use 8px radius, and touch targets stay at or above 44px.
- **Auth navigation**: Login shows `Quên mật khẩu?` below the password field and `Chưa có tài khoản? Đăng ký` below the form. Register shows `Đã có tài khoản? Đăng nhập` below the form.
- **Google button**: When Google is configured, the Google Identity Services button must align to the same width as the primary submit button. When not configured, the UI must avoid broken provider prompts and must not ask for raw credentials.
- **Validation and toast UX**: Auth forms use frontend Zod validation and disable browser-native validation copy. Validation, success, and recoverable API errors appear as concise Vietnamese React Toastify messages.
- **Cooldown UX**: Verification-code and reset-code resend controls show a visible countdown such as `Gửi lại sau 60s`, stay disabled during cooldown, and return to the normal action label when available.
- **Session UX**: After authentication, route by backend role to `/learner`, `/mentor`, or `/admin`. These pages are placeholder dashboards for showing safe current-user data only.
- **State rule**: Frontend Auth state comes from `AuthProvider` and `/auth/me`; never store JWTs, refresh tokens, cookie values, OTPs, password hashes, or Google tokens in browser storage.
