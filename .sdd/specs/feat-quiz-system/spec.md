...
**Input**: User description: "Create the OCP Quiz System feature specification using 001-feat-auth as the example style, covering learner access to active quizzes in accessible courses or lessons, backend-controlled quiz eligibility, secure question delivery, answer submission, automatic scoring for supported question types, result display, quiz submission history, module boundaries, and explicit out-of-scope boundaries."
...
3. **Given** a quiz is in draft or inactive status, **When** a learner requests available quizzes for a course they can access, **Then** the quiz is not shown as available to take.
...
### User Story 6 - Preserve Quiz Module Boundaries And Assessment Integrity (Priority: P3)
...
- Draft or inactive quizzes must not be available to learners regardless of course access.
...
- Quiz authoring, question management, and admin content management must not be assumed in this learner-facing MVP.
...
- **FR-005**: The system MUST return only active quizzes as available for learner-facing quiz listing and MUST exclude draft or inactive quizzes.
...
- **Quiz**: An assessment attached to a course and optionally to a lesson. It includes title, description, optional time limit in minutes, passing score threshold, status that controls learner availability, and timestamps.
- **Quiz Question**: A scored question belonging to a quiz. It includes question text, question type (`single_choice`, `multiple_choice`, `true_false`), answer options, correct answer for scoring, optional explanation, point value, display order, and timestamps. Correct answer and explanation are protected before submission.
...
- **Quiz Availability View**: The learner-facing safe representation of active quizzes available for an accessible course or lesson, including quiz metadata and the learner's own attempt summary without revealing correct answers.
...
- Quiz authoring UI, quiz CRUD, question CRUD, bulk question import, question reordering, or admin content management.
...
- Active quizzes and questions already exist through seed data, an approved admin or content workflow, or a later quiz-authoring feature; authoring is not part of this learner-facing MVP.
