...
- **WHEN** Manager truy cập hàng đợi bằng GET `/manager/courses/pending-review` hoặc GET `/manager/reviews/pending`, **THE hệ thống SHALL** trả về danh sách các `course_revisions` có trạng thái `pending_review`, sắp xếp theo thời gian nộp mới nhất trước.
...
- **WHEN** Manager bấm duyệt bằng POST `/manager/reviews/:revisionId/publish` hoặc POST `/manager/reviews/:revisionId/approve`, **THE hệ thống SHALL**:
...
- **WHEN** Manager yêu cầu danh sách category bằng GET `/manager/categories`, **THE hệ thống SHALL** trả về danh sách các danh mục khóa học hiện có.
- **WHEN** Manager thực hiện tạo/sửa/xóa category bằng POST/PUT/DELETE `/manager/categories`, **THE hệ thống SHALL** kiểm tra quyền và thực thi cập nhật bảng danh mục nếu được cấu hình.

---

## 4. Non-functional Requirements

- **Access Control**: Chặn tất cả request không có role `MANAGER` hoặc có token bị mismatch `role_version` (E2.2).
...
- **WHERE** Một Manager có token hết hạn hoặc quyền thay đổi gọi API, **THE hệ thống SHALL** trả về HTTP 401 Unauthorized do mismatch `role_version` (E2.2).
