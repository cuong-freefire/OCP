...
- `getManagerDashboardStats()`: Tổng hợp số liệu tổng quan từ `PaymentService`, `LearningService` và `MentorService`.
- `getCourseMetricsDetail(courseId)`: Chi tiết lượt enroll, doanh thu, rating, completion của khóa học.
- `getRevenueReport(filters)`: Thống kê doanh thu theo bộ lọc thời gian và khóa học.
- `getMentorsReport()`: Tổng hợp hiệu suất kinh doanh và giảng dạy của từng Mentor.
- `getAdminDashboardStats()`: Thống kê vĩ mô cho Admin.

#### B. Controller & Routes (`backend/src/controllers/manager.controller.js`, `backend/src/api/manager.routes.js`)
- `getManagerDashboard`: Controller gọi Service tương ứng và trả về kết quả JSON.
- Đăng ký middleware xác thực phân quyền MANAGER / ADMIN và check `role_version`.

### Frontend Components

- **ManagerDashboard.jsx:** UI hiển thị biểu đồ doanh thu, số học viên, danh sách khóa học bán chạy. Giao diện thiết kế theo style glassmorphism thời thượng.
- **CourseMetricsDetail.jsx:** UI chi tiết số liệu của một khóa học cụ thể.
- **ManagerReports.jsx:** Trang lọc và hiển thị báo cáo dạng bảng chuyên nghiệp (hỗ trợ phân trang, sắp xếp).
- **AdminDashboard.jsx:** UI thống kê macro cho Admin.
