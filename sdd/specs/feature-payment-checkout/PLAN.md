...
1. Service commit transaction.
2. Controller trả response thành công:
...
1. Frontend redirect learner sang `vnpayPaymentUrl`.
2. Checkout flow kết thúc ở `PENDING`. Không verify VNPAY, không cập nhật `SUCCESS/FAILED`, không tạo `enrollment`.
