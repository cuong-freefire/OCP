<<<<<<< Updated upstream:OCP/agent/AGENTS.md
# Version: 2.0 | Updated: 2026-05-30 | Project: Online Course Platform (OCP)
=======
# Version: 4.0 | Updated: 2026-06-09 | Project: Online Course Platform (OCP)
>>>>>>> Stashed changes:AGENTS.md
...
4. File này — quy tắc vận hành cụ thể cho agent
>>>>>>> Stashed changes:AGENTS.md
...
=======
Backend: NodeJS + JavaScript ESM + Express-style REST API trong `backend` + Chỉ dùng component function không dùng class
Frontend: React + JSX + Bootstrap + Create React App (`react-scripts`) trong `frontend` + Chỉ dùng component function không dùng class
>>>>>>> Stashed changes:AGENTS.md
Database: MySQL
ORM: Prisma
...
=======
Package manager: npm
Testing: backend `node:test` + `assert` + `supertest`; frontend `node:test` cho API client/source-rule tests nhẹ

Không được tự ý đổi stack sang TypeScript, CommonJS, Vite, Next.js, ORM/database/payment gateway khác, hoặc Bearer-token auth nếu chưa có human approval.
>>>>>>> Stashed changes:AGENTS.md
...
```
=======
- API style: REST theo prefix/config hiện hành của backend
- Controller chỉ nhận request, gọi service, và trả response; không chứa business logic
- Service chứa business logic và module orchestration; không import Prisma trực tiếp
- Repository là tầng duy nhất được import Prisma và thao tác database
- Cross-module access phải đi qua service/contract của module sở hữu dữ liệu, không query trực tiếp table của module khác
- Backend là source of truth cho identity, role, userId, payment status, enrollment, và course access
- Frontend guard chỉ là UX, không thay thế backend authorization
- Error response phải thống nhất theo `{ success, message, code, details }` hoặc response helper đã duyệt
- Không trả raw stack trace, Prisma raw error, SQL error, secret, JWT, cookie value, password, password hash, OTP ra frontend
...
Backend utilities: `[purpose].util.js`
Frontend API client: camelCase `.js` trong `frontend/src/api` (e.g. `authApi.js`)
Frontend components/pages: PascalCase `.jsx` (e.g. `CourseDetailPage.jsx`)
Frontend hooks/utilities: camelCase `.js` (e.g. `usePaymentCheckout.js`, `formatCurrency.js`)
Frontend env variables: dùng prefix `REACT_APP_`
Database tables: snake_case (e.g. `payments`, `payment_events`, `enrollments`)
Specs: `.sdd/specs/[number]-[feature-name]/`
>>>>>>> Stashed changes:AGENTS.md
...
=======
- Đọc `PROJECT_AGENTS.md`, `constitution.md`, `DATABASE.md`, feature spec/plan/tasks và code liên quan trước khi chỉnh sửa
...
2. Backend đọc token từ `req.cookies`
3. Frontend gửi request kèm cookie bằng `withCredentials: true` hoặc credentials option tương đương
...
7. Payment/enrollment behavior phải idempotent và không tạo duplicate state
>>>>>>> Stashed changes:AGENTS.md
...
| Mentor Course Studio + Revision | Member 2 - Nam | Sở hữu course creation, revision, submit review; Module khác lấy course price/status qua contract |
| Payment + Enrollment + Catalog | Member 3 - CuongLH | Sở hữu payments, orders, enrollments, catalog, course access |
| Learning + Quiz + Rating | Member 4 - Đức | Dùng Access contract, không tự đọc payments để unlock; Rating/feedback chỉ cho enrolled learners |
| Manager Approval + Admin | Member 5 - Tiến | Phê duyệt/reject course revision, admin quản lý user/reports |

### Soft delete rules

- User/course/payment/enrollment/submission quan trọng không được hard-delete nếu spec không yêu cầu rõ
- Course inactive/deleted không được bán qua checkout
- Transaction/payment history phải được giữ để audit
- Không xóa vĩnh viễn dữ liệu nghiệp vụ quan trọng nếu không có yêu cầu rõ từ spec
...
- Nếu yêu cầu mơ hồ hoặc thiếu domain context quan trọng, hỏi lại thay vì đoán
- Nếu spec, code hiện có, `PROJECT_AGENTS.md`, `DATABASE.md`, hoặc file này mâu thuẫn, phải báo conflict trước khi implement
- Luôn kiểm tra tác động nghiệp vụ trước khi sửa flow auth, payment, enrollment, access control hoặc database
- Trước thay đổi có rủi ro cao, phải đọc code liên quan trong `PROJECT_AGENTS.md`, spec hiện hành, và module lân cận
...
Pending: follow active workflow state in `PROJECT_AGENTS.md`, `.specify/workflows/`, and `.sdd/specs/`
>>>>>>> Stashed changes:AGENTS.md
...
<!-- SPECKIT START -->
Active Speckit plan: `.sdd/specs/002-feat-profile/plan.md`
Feature: `feat-profile`
Generated: 2026-06-05 via `/speckit-plan`
<!-- SPECKIT END -->
...
- `share_context.md` — project flow, module ownership, cross-module contracts
- `.specify/workflows/` — approved workflow definitions
- `.sdd/specs/[feature-name]/` — generated feature artifacts
- `frontend/DESIGN.md` — frontend visual/UX contract khi chỉnh frontend
- `chia job.docx` — phân chia module, API ownership, database ownership theo thành viên
- `spec-driven-&-agent-driven-development.pdf` — quy trình Spec-Driven Development và Agent-Driven Development
>>>>>>> Stashed changes:AGENTS.md
